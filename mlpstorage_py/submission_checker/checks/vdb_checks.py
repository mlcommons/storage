"""VdbCheck — Rules.md §5 (Vector Database) extension stub.

Rules.md §5 is empty at Phase 3 land time. STUB-01 establishes the
...
class VdbCheck(BaseCheck):
    """Stub check class for Rules.md §5 (Vector Database) rules.
    ...
    instantiate ``VdbCheck`` without any special-casing.

    Emits zero violations. Future phase populates ``STUB_COVERAGE['VdbCheck']``
    in ``coverage_mapping.py`` when Rules.md §5 gains IDs.
    """
    ...
    def init_checks(self):
        ...
        Rules.md §5 (VDB) is empty at Phase 3 land time. When that section
        gains IDs, a future phase fills in real ``@rule``-decorated check
        methods here and populates ``STUB_COVERAGE['VdbCheck']`` in
        ``coverage_mapping.py``.
        """
