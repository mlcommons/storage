...
class CheckpointSubmissionRulesChecker(MultiRunRulesChecker):
    ...
    def check_invocation_structure(self) -> List[Issue]:
        ...
        Any other arrangement (e.g. ten 1-write runs, overlapping phases,
        or a >30s gap between the write and read phases) is INVALID for
        CLOSED. (The v3.0 round reported the gap breach as a
        warning-severity issue — special build PR #834, then worklist A7;
        retired when the round closed.)
        """
        issues = []
