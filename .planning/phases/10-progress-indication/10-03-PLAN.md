...
must_haves:
  truths:
    ...
    - "Error handling is preserved when operations are wrapped in progress"
  artifacts:
...
**IMPORTANT: Error Handling Preservation**

All existing try/except blocks MUST remain intact when wrapping operations in progress_context. The progress context manager is additive - it does not replace error handling. Exceptions raised inside progress_context will propagate correctly after the context manager cleans up the progress display.
...
2. Update run_benchmark() to show progress during lockfile validation.
   PRESERVE existing try/except error handling - wrap the try block, not just the call:
   ```python
   ...
   if hasattr(args, 'verify_lockfile') and args.verify_lockfile:
       with progress_context(
           ...
           try:
               ...
               # This exception MUST still be raised after progress cleans up
...
3. Update run_benchmark() to show progress during environment validation.
   PRESERVE existing error handling - validate_benchmark_environment may raise:
   ```python
   ...
   ```python
       ...
       with progress_context(
           ...
           try:
               ...
               # Error handling preserved - exceptions propagate
               logger.error(f"Lockfile not found: {lockfile_path}")
               return 1
   ```

**Verification of error handling preservation:**
- FileNotFoundError for missing lockfile still raises/returns appropriately
- ValidationIssue exceptions from validate_benchmark_environment still propagate
- All existing return codes for error cases are maintained
  </action>
  <verify>python -c "from mlpstorage.main import main, run_benchmark; print('Import OK')"</verify>
  <done>main.py shows progress spinners during validation and lockfile operations, all error handling preserved</done>
</task>
...
2. Test error handling still works (lockfile not found):
   ```bash
   mlpstorage lockfile verify /nonexistent/lockfile.txt
   ```
   Expected: See "Verifying lockfile..." spinner briefly, then clear error message about file not found (error should still be raised)

3. Test lockfile generation progress (if uv available):
   ```bash
...
4. Test lockfile verify progress:
   ```bash
...
5. Test non-interactive mode (redirected output):
   ```bash
...
6. Verify no interference with DLIO output:
   - If DLIO is installed, run actual benchmark briefly
   - DLIO's own progress should display cleanly without interference
  </how-to-verify>
  <resume-signal>Type "approved" if progress indicators work correctly in interactive and non-interactive modes AND error handling is preserved (test #2 shows proper error), or describe any issues observed</resume-signal>
</task>
...
4. Error handling test: `mlpstorage lockfile verify /nonexistent/file.txt` still shows error
5. Human verified progress indicators work visually
</verification>
...
5. All existing error handling preserved (FileNotFoundError, validation errors still work)
6. User confirms visual progress indicators are working
</success_criteria>
