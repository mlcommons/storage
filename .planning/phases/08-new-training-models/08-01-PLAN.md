...
must_haves:
  truths:
    ...
    - "DLIO loads configuration for each model-accelerator combination"
  artifacts:
...
<task type="auto">
  <name>Task 3: Create RetinaNet and Flux YAML configurations</name>
  <files>
    configs/dlio/workload/retinanet_h100.yaml
    configs/dlio/workload/retinanet_a100.yaml
    configs/dlio/workload/retinanet_datagen.yaml
  ...
  <action>
**RetinaNet (object detection, CNN):**
...
**Flux (text-to-image diffusion, transformer):**
...
- workflow: generate_data=True, train=False, checkpoint=False
  ...
  <verify>
Run: `ls configs/dlio/workload/{retinanet,flux}_*.yaml` - should show 6 files
Run: `grep "model:" configs/dlio/workload/retinanet_h100.yaml configs/dlio/workload/flux_h100.yaml`
  </verify>
  <done>RetinaNet and Flux each have h100, a100, and datagen YAML configs</done>
</task>
