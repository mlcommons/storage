...
Verify check_model_recognized is discovered by the base class:
Run: `python -c "
from unittest.mock import MagicMock
from mlpstorage.rules.run_checkers.training import TrainingRunRulesChecker
from mlpstorage.rules.models import BenchmarkRun, BenchmarkRunData
...
data = BenchmarkRunData(
    ...
    num_processes=8,
    parameters={'dataset': {'num_files_train': 100}, 'reader': {}},
    override_parameters={}
)
...
assert 'check_model_recognized' in method_names, f'check_model_recognized not found in {method_names}'
print('check_model_recognized is in check_methods list: PASS')
"`
  </verify>
  <done>TrainingRunRulesChecker imports new model constants, has check_model_recognized method, and method is discovered by base class</done>
</task>
...
class TestTrainingRunRulesCheckerNewModels:
    ...
    def create_benchmark_run(self, model, parameters=None, override_parameters=None):
        ...
        cluster_info = ClusterInformation([host_info], self.mock_logger)
    ...
    def test_new_model_recognized(self, mock_logger, model):
        ...
        self.mock_logger = mock_logger  # For create_benchmark_run helper
    ...
    def test_check_model_recognized_in_check_methods(self, mock_logger):
        ...
        self.mock_logger = mock_logger
        benchmark_run = self.create_benchmark_run("dlrm")
        checker = TrainingRunRulesChecker(benchmark_run, logger=mock_logger)
...
Expected: All 11 tests pass (3 models x 3 test methods + 1 unrecognized test + 1 discovery test)
  </verify>
  <done>Unit tests for new model validation pass, including verification that check_model_recognized is discovered</done>
</task>
...
3. check_model_recognized is in checker.check_methods list (auto-discovered)
4. New models work with existing validation (no special requirements beyond existing rules)
5. All new unit tests pass
6. Existing tests still pass: `pytest tests/unit/test_rules_checkers.py -v`
</verification>

<success_criteria>
- `grep "DLRM" mlpstorage/rules/run_checkers/training.py` shows import
- `pytest tests/unit/test_rules_checkers.py -v -k "TestTrainingRunRulesCheckerNewModels"` shows 11 passing tests
