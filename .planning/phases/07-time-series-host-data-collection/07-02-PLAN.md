...
<verification>
1. Import check: `python -c "from mlpstorage.cluster_collector import MultiHostTimeSeriesCollector, TIMESERIES_SSH_SCRIPT; print('OK')"`
...
3. Localhost hosts use direct collection (no SSH)
4. Remote hosts use SSH with TIMESERIES_SSH_SCRIPT
5. Collection continues even when some hosts fail
6. Samples are organized by hostname in stop() return value
