...
```
  ...
  <done>
validate_benchmark_environment is called in run_benchmark before benchmark instantiation. Validation errors are caught by existing exception handlers in main().
  </done>
</task>
...
```
  ...
  <done>
Benchmark base class has _validate_environment hook that is called at start of run(). Subclasses can override for benchmark-specific validation.
  </done>
</task>
...
2. Benchmark base class has _validate_environment hook
3. _validate_environment is called at start of run()
4. Validation errors are caught and displayed with suggestions
5. All tests pass
6. Validation happens before cluster info collection
</success_criteria>
