...
2. Add `check_mpi_with_hints(mpi_bin: str = "mpirun") -> str`:
   ...
   - If not found:
     - Call `os_info = detect_os()` to get OSInfo dataclass
     - Call `install_cmd = get_install_instruction("mpi", os_info)` to get OS-specific command
     - Raise DependencyError using format_error('DEPENDENCY_MPI_MISSING', install_cmd=install_cmd)
...
3. Add `check_dlio_with_hints(dlio_bin_path: Optional[str] = None) -> str`:
   ...
   - (DLIO is pip-installed, so no OS detection needed - just pip install commands)

4. Add `check_ssh_available() -> str`:
   - Check for `ssh` executable in PATH using `shutil.which('ssh')`
   - If found, return path
   - If not found:
     - Call `os_info = detect_os()` to get OSInfo dataclass
     - Call `install_cmd = get_install_instruction("ssh", os_info)` to get OS-specific command
     - Raise DependencyError using format_error('DEPENDENCY_SSH_MISSING', install_cmd=install_cmd)
...
```
  ...
  <done>
check_mpi_with_hints, check_dlio_with_hints, and check_ssh_available functions exist. When dependencies are missing, they call detect_os() to get OSInfo, then call get_install_instruction() with the dependency name and OSInfo to get the install command, then raise DependencyError with OS-specific installation commands.
  </done>
</task>
...
assert 'ENVIRONMENT_VALIDATION_SUMMARY' in ERROR_MESSAGES
# Test templates work with format_error
msg = format_error('DEPENDENCY_MPI_MISSING', install_cmd='sudo apt-get install openmpi-bin')
assert 'apt-get' in msg
msg = format_error('DEPENDENCY_SSH_MISSING', install_cmd='sudo apt-get install openssh-client')
assert 'apt-get' in msg
msg = format_error('ENVIRONMENT_VALIDATION_SUMMARY', error_count=3, error_list='- Error 1\n- Error 2\n- Error 3')
assert '3' in msg
print("All templates format correctly")
```
  </verify>
  <done>
New error message templates added. Templates include placeholders for OS-specific install commands and are formatted for readability. format_error() works correctly with all new templates.
  </done>
</task>
...
2. Functions call detect_os() and get_install_instruction() to get OS-specific commands
3. Functions raise DependencyError with OS-specific install commands when deps missing
4. New error templates exist in error_messages.py
5. format_error() works correctly with all new templates
6. All tests pass (pytest tests/unit/test_dependency_check.py)
7. Error messages are multi-line, readable, and include copy-pasteable commands
</success_criteria>
