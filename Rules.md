...
**Rules editions and comparability classes** -- A *rules edition* is the version of this document a result was produced under ("3.0" today; the value every `provenance.json` and `submission.yaml` declares).  An edition bumps only when a workload's numbers or validity change, or a workload is added or retired; tool releases never bump it.  Which editions exist, what each sanctioned (division, family, model, emulated accelerator), which DLIO revisions and client storage libraries its rows ran on, and which results repository holds them is recorded in the machine-readable table `mlpstorage_py/rules/editions.yaml` (`schema: mlps-rules-editions/1`), maintained by the working group by pull request.  Its `current_edition` must equal the edition the tool stamps.

The same table declares the *comparability classes*: a class is the working group's assertion that runs of one (family, model, emulated accelerator) whose `core-config-v1` hash is one of the class's listed hashes executed the same workload in every edition the class lists.  Only rows in one class are comparable, within a round or across rounds; nothing is inferred.  A class may list several hashes (the same workload spelled differently by different DLIO generations, e.g. v1.0 `dataset.record_length` versus v2.0 `dataset.record_length_bytes`) and several editions (e.g. llama3-8b checkpointing is one class across 2.0 and 3.0).  Checkpointing classes use `accelerator: any` because the checkpoint parameters do not vary with the emulated accelerator.  DLIO revisions and storage libraries are not part of the class key; they are recorded per edition as accepted lists.  Families without a core-config allowlist (kv_cache, vector_database) have no classes yet.  The class is never written into a leaf or manifest -- it is looked up from the stamp and the table (`mlpstorage runs show` prints it).  The *submission validator* enforces:
- EDN-01 (`rulesEdition`): a `provenance.json` or `submission.yaml` declaring a rules edition the table does not know fails.
- EDN-02 (`comparabilityClass`): a stamped `run` leaf whose (edition, family, model, emulated accelerator, core-config hash) resolves to no class fails under `closed/` and is reported as information under `open/` and `whatif/`; datasize/datagen leaves and families without an allowlist are skipped.
- EDN-03 (`dlioRevision`): a stamped `run` leaf whose DLIO commit is not in its edition's accepted list is reported as information, so review chairs can extend the table.
Leaves whose stamp is derived (no `provenance.json`, edition "unknown") draw no finding under any of the three.
