...
- "core_config": {"algorithm": "core-config-v1", "allowlist", "hash", "keys"} -- a SHA-256 (first 16 hex digits) over the canonical JSON of the workload-defining DLIO parameters, keeping only the keys in the named allowlist (`mlpstorage_py/rules/core_config_keys.yaml`, e.g. "checkpointing@1").  Site tunables (the "Tunable Parameters for CLOSED" tables, paths, thread counts, listing and storage knobs) are excluded, so two runs of the same workload hash alike wherever they ran; "keys" lists exactly what was hashed so the value is reproducible from the leaf alone.  Every family has an allowlist: `training@1` and `checkpointing@1` over the resolved DLIO parameters; `kv_cache@1` over the three Table KVCache-1 Options as emitted to `kv-cache.py`, the 6.3.2.1 seed and config, and the global flags forwarded; `vector_database@1` over the backend, index family, dataset scale (`num_vectors` x `dimension`) and recall target (`recall_k`, `search_limit`).  A leaf written before its family's block existed in `*_metadata.json` is rebuilt at read time from what it recorded (the wrapper command lines; `summary.json` / `config.json` / `result_verdict.json`), and stamps "unknown" when that is not enough.
...
**Run-leaf and results-table integrity** -- Two further checks make failures that `reportgen` would otherwise absorb visible at validation time.
- LEAF-01 (`runLeafMetadata`): a `kv_cache` or `vector_database` `run` leaf must carry exactly one `<benchmark-type>_<timestamp>_metadata.json`.  Nothing else identifies a run of those two benchmarks (training and checkpointing leaves identify themselves through their DLIO configuration), so a leaf without the file -- or with more than one -- is one `reportgen` drops, and the workload's metric columns publish blank.  The finding names the file to restore; the measured data need not be regenerated.
...
**Rules editions and comparability classes** -- A *rules edition* is the version of this document a result was produced under ("3.0" today; the value every `provenance.json` and `submission.yaml` declares).  An edition bumps only when a workload's numbers or validity change, or a workload is added or retired; tool releases never bump it.  Which editions exist, what each sanctioned (division, family, model, emulated accelerator), which DLIO revisions and client storage libraries its rows ran on, and which results repository holds them is recorded in the machine-readable table `mlpstorage_py/rules/editions.yaml` (`schema: mlps-rules-editions/1`), maintained by the working group by pull request.  Its `current_edition` must equal the edition the tool stamps.  Each edition this tool can check carries a `checker` block -- the files and folders every datagen, run and checkpoint leaf of that edition must contain, the minimum AU per training model (3.3.2), the Table 2 CLOSED process count and checkpoint size per model (4.6.1, 4.3.4), the Table 3 simulated accelerator memory (4.3.4) and the KVCache sequence locks (6.3.2.1) -- and `validate` checks each submission with the block of the edition its `submission.yaml` declares (a submission without a manifest is checked under the current edition), and only the checks bound to that edition run.  A historical edition listed without a `checker` block is checked by the tool of its own round.

The same table declares the *comparability classes*: a class is the working group's assertion that runs of one (division, family, model, emulated accelerator) whose `core-config-v1` hash is one of the class's listed hashes executed the same workload in every edition the class lists.  Only rows in one class are comparable, within a round or across rounds; nothing is inferred.  Comparisons across divisions are never declared: a class belongs to exactly one division, and an OPEN run that happens to use a CLOSED configuration is not in the CLOSED class.  A class may list several hashes (the same workload spelled differently by different DLIO generations, e.g. v1.0 `dataset.record_length` versus v2.0 `dataset.record_length_bytes`) and several editions (e.g. llama3-8b checkpointing is one class across 2.0 and 3.0).  Checkpointing classes of editions 3.0 and earlier use `accelerator: any` because those rounds recorded no emulated accelerator for checkpoint runs; from edition 4.0 on checkpoint classes are per accelerator (b200 and mi355 checkpoint rows do not compare).  For editions 3.0 and earlier, DLIO revisions and storage libraries are not part of the class key; they are recorded per edition as accepted lists.  From edition 4.0 on the working group may bring the DLIO revision into the key.  kv_cache and vector_database classes use `accelerator: any` permanently (neither family emulates an accelerator); the edition-3.0 seed is the Table KVCache-1 sequence (`llama3.1-8b-A`) and Milvus DISKANN with recall@10 at 1M x 1536 (`milvus-diskann-A`, the shipped `configs/vectordbbench/default.yaml`) and at 10M x 1536 (`milvus-diskann-B`, the `10m.yaml` scale).  The class is never written into a leaf or manifest -- it is looked up from the stamp and the table (`mlpstorage runs show` prints it, and `reports reportgen` emits it beside the rules edition in the `Rules Edition` and `Comparability Class` columns of results.csv: a row shows the edition and class that every one of its run leaves resolves to -- an unstamped leaf takes its edition from its submission's `submission.yaml` -- and a blank class when its leaves fall into different classes or only some of them classify, because such a row is not comparable).  The *submission validator* enforces:
...
- EDN-03 (`dlioRevision`): a stamped `run` leaf whose DLIO commit is not in its edition's accepted list draws a warning, so review chairs can extend the table.
...
3.3.8. **trainingResultAggregation** -- The training figures published for a submission (the `Read B/W (GiB/s)` column of `results.csv`, and the `train_mean_of_*` metrics in the "run" *phase directory*'s `results.json`) are computed by `mlpstorage reports reportgen` from the 5 measured *timestamp directories* of §2.1.17; the first (*warm up*) invocation is excluded and no other invocation is excluded, so no invocation may be discarded as an outlier.  Within one invocation the benchmark records one value per epoch: the per-epoch AU is the mean across all ranks of each rank's AU (§3.3.2), and the per-epoch throughput (samples/s) is the sum across all ranks.  The benchmark reports the arithmetic mean over that invocation's epochs as `train_au_mean_percentage` and, multiplied by the sample size, `train_io_mean_MB_per_second` in the invocation's `*summary.json` (that field is MiB/s despite its name).  Across invocations every published figure is the **arithmetic mean of the 5 per-invocation means**: no minimum, maximum, median, or geometric mean is used.  `Read B/W (GiB/s)` is the mean of the 5 `train_io_mean_MB_per_second` values divided by 1024.  The §3.3.2 AU minimum is applied to each invocation individually by the *submission validator*; the aggregated AU is informational.  A list-valued metric (per-epoch AU, per-epoch throughput) that is empty in any of the 5 invocations makes the workload INVALID rather than being averaged over a subset; `Read B/W (GiB/s)` is published blank if any invocation lacks `train_io_mean_MB_per_second`.
...
4.3.6. **checkpointResultAggregation** -- The checkpointing figures published for a submission (the `Write B/W (GiB/s)`, `Write Duration (secs)`, `Read B/W (GiB/s)`, and `Read Duration (secs)` columns of `results.csv`) are computed by `mlpstorage reports reportgen` from the one or two *timestamp directories* of §2.1.23; there is no *warm up* invocation and no invocation is excluded.  For each of the 10 checkpoints written (respectively read) in an invocation the benchmark records the duration of that checkpoint in seconds and its rate, `checkpoint_size_GB ÷ duration`, where `checkpoint_size_GB` is the checkpoint size summed across all ranks in binary GiB (see the units note under Table 2), so the rate is the aggregate bandwidth across all processes in GiB/s.  The benchmark reports the arithmetic mean over the 10 as `save_checkpoint_io_mean_GB_per_second` and `save_checkpoint_duration_mean_seconds` (respectively `load_checkpoint_io_mean_GB_per_second` and `load_checkpoint_duration_mean_seconds`) in the invocation's `*summary.json`.  The published bandwidth is therefore the **mean of the 10 per-checkpoint rates**, not the total bytes divided by the total elapsed time; the two differ when checkpoint durations vary.  Each column is taken from the invocation(s) configured to perform that phase, `checkpoint.num_checkpoints_write > 0` for the write columns and `checkpoint.num_checkpoints_read > 0` for the read columns: a single combined invocation supplies all four columns, and a §4.7.1 write-then-read split supplies the write columns from the first invocation and the read columns from the second.  If more than one invocation performed the same phase, the column is the arithmetic mean of their per-invocation means.  A column whose field is missing from an invocation that was configured to produce it is published blank rather than as a mean over a subset.
