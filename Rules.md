...
1.3. **runResultSubmission** -- Three nouns name the parts of a package throughout this document.  A *run* is one invocation of a benchmark by the `mlpstorage` command: one *timestamp directory* together with its contents (2.1.13, 2.1.17, 2.1.23, 5.3.1, 6.3).  A *result* is the set of runs under one *workload directory* of one *system name* directory, within one division hierarchy, that share the same benchmark, workload and emulated accelerator; the rules of §3 to §6 reduce a result to one row of `results.csv` (3.3.8, 4.3.6, 6.3.4).  The number of runs a complete result holds is recorded per benchmark for the submission's rules edition in the `checker` block of `mlpstorage_py/rules/editions.yaml` (`runs_per_result`; edition 3.0: training 6 per 2.1.17, checkpointing 2 -- the write phase and the read phase of 4.7.1, which one combined invocation supplies together -- vector database 5 per 5.3.1, KVCache 1).  A *submission* is every result of one submitter within one division hierarchy (2.1.4), together with that submitter's "systems" directory (2.1.7) and "submission.yaml".  (Commentary: `RulesCommentary.md` §1.3.)
