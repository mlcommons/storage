...
- A `run` also copies the manifest it consumed into its own leaf as `datagen-manifest.json` (see below) and names it in the metadata as `"datagen_manifest_file"`, so the results tree carries the dataset's description without any reference back to `--data-dir`.
...
**The run-leaf snapshot (`datagen-manifest.json`).** `mlpstorage validate` never inspects `--data-dir`, so the manifest itself plays no part in submission validation. What `validate` sees is the copy `run` leaves in its results leaf: `<run-leaf>/datagen-manifest.json`, schema `mlps-datagen-manifest-snapshot/1`, holding the manifest body verbatim under `"manifest"`, where it was read from (`"location"`), when (`"consumed_at"`) and which `MANIFEST-*` findings `--skip-validation` or `whatif` waived (`"overridden"`, normally empty). The leaf's `*_metadata.json` declares it as `"datagen_manifest_file": "datagen-manifest.json"`. `configview` writes no leaf and so no snapshot. Rule 3.3.1 uses the snapshot to tie the run to the `datagen/` leaf that produced its data, entirely inside the submission tree:

| Token | Level | Condition |
|---|---|---|
| `[3.3.1 MANIFEST-MISSING]` | error | The metadata declares a snapshot but the file is absent or unreadable. |
| `[3.3.1 MANIFEST-OVERRIDDEN]` | error | The run proceeded past `MANIFEST-001` / `MANIFEST-003` under `--skip-validation` or `whatif`: the dataset did not fit the run as configured. |
| `[3.3.1 MANIFEST-LINK]` | error | The manifest's `source_datagen_result_dir` is not one of this submission's `datagen/<timestamp>/` leaves: the data was generated by some other datagen. |
| `[3.3.1 MANIFEST-COUNT]` | error | The manifest's `num_files_train` differs from what that `datagen/` leaf recorded. |
| `[3.3.1 MANIFEST-OVERRUN]` | error | The run's `num_files_train` exceeds the manifest's generated count. |
| `[3.3.1 MANIFEST-ABSENT]` | warning | A stamped leaf (one with `provenance.json`) that consumed no manifest: nothing to link; the run's data provenance rests on the `datasize/` / `datagen/` / `run/` leaves alone. |

Leaves that declare neither `provenance_file` nor `datagen_manifest_file` (every leaf of the v3.0 round) are silent. Checkpointing does not use `--data-dir` and neither writes nor reads a manifest.
...
└── run/
    ...
    └── <YYYYMMDD_HHMMSS>/                one per run; closed requires 6
        ...
        ├── datagen-manifest.json         the datagen manifest this run consumed; see DATA DIRECTORY
        ├── training_<ts>_timeseries.json metrics; absent if --skip-timeseries
        └── training_<ts>_metadata.json   args, env, cluster info, status
```

Rule 3.3.1 (`trainingRunDataMatchesDatasize`) reads the `datasize/` metadata to enforce `datasize.num_files_train ≤ run.num_files_train ≤ datagen.num_files_train`, and reads each run leaf's `datagen-manifest.json` to tie the run to the `datagen/` leaf that produced its data (`MANIFEST-LINK` / `-COUNT` / `-OVERRUN` / `-OVERRIDDEN` / `-MISSING` errors, `MANIFEST-ABSENT` warning; DATA DIRECTORY → "The datagen manifest"). Note that `datagen/` no longer carries `*output.json` / `*per_epoch_stats.json` / `*summary.json` — those are training-loop artifacts that DLIO does not produce when `workflow.generate_data=True, workflow.train=False`.
...
`reportgen` emits one `results.json` and one `results.csv` per model folder, at the canonical `<benchmark>/<model>/` group boundary. Every row carries, after `Model`, the `Rules Edition` and `Comparability Class` columns the unified results archive keys on: the edition every contributing run leaf's provenance stamp declares (an unstamped leaf falls back to its submission's `submission.yaml`) and the comparability class `rules/editions.yaml` assigns that (division, family, model, accelerator, core-config hash) under that edition -- or, when no edition is known, by hash across every edition, as `runs show` prints it. Both cells are blank when the leaves disagree (a warning names the leaves) or when nothing is known; datagen and datasize leaves never contribute. Grouping is the parent of `<command>/` (or, for checkpointing which omits the `<command>` segment, the parent of `<ts>/` — which is `<model>/` itself). A single reportgen invocation therefore produces multiple sibling rollup files, one per benchmark model discovered under the results-dir slice.
...
There is no edition flag. Each submission is checked with the parameters of the rules edition its `submission.yaml` declares (`rules_edition`), read from the `checker:` block of that edition in `mlpstorage_py/rules/editions.yaml` -- the required leaf contents, the minimum AU per training model (3.3.2), the Table 2 CLOSED process counts and checkpoint sizes, the Table 3 simulated accelerator memory (4.3.4), the KVCache 6.3.2.1 sequence locks and the runs per result of each benchmark (Rules.md 1.3, `runs_per_result`: the 2.1.17 and 5.3.1 counts); a submission without a manifest is checked under the current edition. A manifest declaring an edition the table lists without a `checker:` block (a historical round checked by its own tool) fails EDN-04 and that submission's workload checks are skipped. A rule whose logic changed between editions is bound per edition in the checker source (`@rule(..., since=, until=)`, a half-open range), so only the checks bound to that edition run; the others are skipped at debug level.
...
- `<data-dir>/<model>/.mlps-datagen-manifest.json` — written by a completed training `datagen`: model, file count, samples per file, record length, subfolder count, format, rules edition, provenance. Read by training `run` / `configview` (`MANIFEST-000/001/003`); never read by `validate`, which sees the run leaf's copy instead. See DATA DIRECTORY.
- `<results-dir>/<mode>/<orgname>/results/<systemname>/training/<model>/run/<YYYYMMDD_HHMMSS>/datagen-manifest.json` — the manifest that run consumed, copied into the leaf (declared in the metadata as `datagen_manifest_file`); `validate` rule 3.3.1 links the run to its `datagen/` leaf through it. See DATA DIRECTORY → "The datagen manifest".
