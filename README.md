...
Systems with container engine can use the following instructions to run the benchmark in a container
...
```bash
docker build -t mlcommons/storage:v0.5 .
```

Verify configuration.
```bash
docker run mlcommons/storage:v0.5 -h
Usage: /workspace/storage/benchmark.sh [datasize/datagen/run/configview/reportgen] [options]
Script to launch the MLPerf Storage benchmark.
```

Using following instructions, all `./benchmark.sh` commands can be replaced with `docker run mlcommons/storage:v0.5` like:
```bash
docker run mlcommons/storage:v0.5 datasize -h
Usage: /workspace/storage/benchmark.sh datasize [options]
...
When running docker, make sure to bind the `dataset.data_folder` path:

```bash
docker run -v /local/path:/unet3d_data mlcommons/storage:v0.5 datagen --workload unet3d --num-parallel 8 --param dataset.num_subfolders_train=10 --param dataset.data_folder=/unet3d_data
```
...
When running docker, make sure to bind the `dataset.data_folder` and the `--results-dir` path:

```bash
docker run -v /local/path/data:/unet3d_data -v /local/path/results:/uned3d_results mlcommons/storage:v0.5 run --workload unet3d --num-accelerators 4 --results-dir /unet3d_results --param dataset.data_folder=/unet3d_data
```
...
```bash
docker run -v /local/path/results:/sample-results mlcommons/storage:v0.5 reportgen --results-dir  /sample-results/
```
