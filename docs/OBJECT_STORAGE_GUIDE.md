...
Use the same `--data-dir` prefix for `datagen` and `run`. `datagen` writes
`s3://$BUCKET/retinanet/.mlps-datagen-manifest.json` (through `s3dlio`) recording
what it generated; `run` reads it back (one HEAD, one GET) and passes the generated
count to DLIO as `dataset.num_files_generated`, so `dataset.num_files_train` may be
smaller than what was generated and `dataset.num_subfolders_train` no longer has to
...
++workload.dataset.num_files_generated=<count from the datagen manifest>
```

This avoids expensive full-prefix listings against large object stores while still sampling generated files before training starts. With `skip_listing` every rank reconstructs the `{prefix}_{idx}_of_{total}.{format}` names instead of listing them, so `num_files_generated` (the `_of_{total}` suffix, read from the datagen manifest) is what lets `num_files_train` be smaller than the generated set; it is omitted when the prefix carries no manifest, and DLIO then assumes the two counts are equal.
